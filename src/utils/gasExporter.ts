...
var SCHEMAS = {
  "Siswa": ["ID", "NIS", "NISN", "Nama", "Jenis_Kelamin", "Tempat_Lahir", "Tanggal_Lahir", "Kelas", "Jurusan", "Tahun_Masuk", "Status", "Nama_Wali", "Alamat", "HP"],
  "Jenis_Pembayaran": ["ID", "Kode", "Nama", "Jenis", "Tahun_Ajaran", "NominalDefault", "Aktif"],
...
function getTableData(sheetName) {
  ...
  for (var i = 0; i < values.length; i++) {
    ...
    for (var j = 0; j < headers.length; j++) {
      var clientKey = mapSheetFieldToClientKey(headers[j]);
      item[clientKey] = values[i][j];
    }
    result.push(item);
  }
  return result;
...
function mapClientKeyToSheetField(clientKey) {
  var mapping = {
    ...
    "aktif": "Aktif",
    "kodePembayaran": "Kode_Pembayaran",
    "nominal": "Nominal",
    "periode": "Periode",
    "status": "Status",
    "terbayar": "Terbayar",
