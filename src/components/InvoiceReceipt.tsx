...
import { Siswa, Transaksi, DetailTransaksi, Tagihan, JenisPembayaran, SchoolConfig } from '../types';
            ...
            <div className="flex items-center gap-3">
              {school.logoUrl ? (
                <img 
                  ...
                  className="rounded-xl object-contain h-12 w-12 border border-slate-200 shadow-sm"
                  onError={(e) => {
                    // Fallback to text monogram if logo fails to load
                    (e.target as HTMLElement).style.display = 'none';
                    const fallback = document.getElementById('text-monogram-fallback');
                    if (fallback) fallback.classList.remove('hidden');
                  }}
                />
              ) : null}
              <div 
                id="text-monogram-fallback"
                className={`p-2.5 bg-blue-600 text-white rounded-xl font-black tracking-wider text-xl shadow-md flex items-center justify-center h-12 w-12 border-2 border-white ${school.logoUrl ? 'hidden' : ''}`}
              >
                {school.inisial || 'SP'}
              </div>
              <div>
                <h1 className="text-xl font-extrabold text-slate-900 tracking-tight leading-none uppercase">{school.nama}</h1>
                <p className="text-xs text-slate-500 mt-1">{school.alamat}</p>
                <p className="text-[10px] text-slate-400 font-mono mt-0.5">{school.subHeader}</p>
              </div>
            </div>
            ...
            <div className="text-center text-xs space-y-12">
              ...
              {school.kepalaSekolah && (
                <div className="space-y-1 pt-2">
                  <p className="text-slate-450 uppercase tracking-wider text-[8px] font-bold">Mengetahui, Kepala Sekolah</p>
                  <p className="font-bold text-slate-800 text-xs mt-4">{school.kepalaSekolah}</p>
                  <p className="text-[9px] text-slate-400 font-mono">NPSN: {school.npsn || 'N/A'}</p>
                </div>
              )}
            </div>

            <div className="text-center text-xs space-y-12 relative">
              {/* Lunas Stamp mockup */}
              ...
              {school.bendahara && (
                <div className="space-y-1 pt-2">
                  <p className="text-slate-450 uppercase tracking-wider text-[8px] font-bold">Bendahara Sekolah</p>
                  <p className="font-bold text-slate-800 text-xs mt-4">{school.bendahara}</p>
                  <p className="text-[9px] text-slate-400 font-mono">SIPES Bendahara</p>
                </div>
              )}
            </div>
          </div>
