...
import { Siswa, JenisPembayaran, TarifSiswa, Tagihan, Transaksi, DetailTransaksi, User, ConsoleLog, AppSettings } from './types';
...
import {
  ...
  Code, LogOut, Radio, Key, ListFilter, Activity, Trash
} from 'lucide-react';
...
export default function App() {
  ...
  const handleConnectSpreadsheet = () => {
    ...
    logAction('info', 'Menginisiasi simulasi Handshake HTTP GET ke Apps Script Web App...');

    setTimeout(() => {
      // Extract Spreadsheet ID from URL
      let ssName = 'SMAN SIDOARJO LEDGER';
      const match = settings.spreadsheetUrl.match(/\/d\/([a-zA-Z0-9-_]+)/);
      const ssId = match ? match[1] : 'MOCK_SS_ID';
      ...
      logAction('success', `🟢 Connected! Berhasil jabat tangan dengan Google Sheet. ID: ${ssId.substring(0, 15)}...`);
      logAction('info', `Ditemukan tabel aktif pada Spreadsheet: Siswa, Jenis_Pembayaran, Tarif_Siswa, Tagihan, Transaksi, Detail_Transaksi, Users.`);
    }, 1500);
  };
  ...
  const handlePushCloudData = () => {
    logAction('info', 'Memulai pengunggahan [PUSH] cache transaksi heriter lokal dan sinkronisasi status lunas...');
    setTimeout(() => {
      logAction('success', '🟢 [SYNC OK] Data transaksi kasir, tarif, dan tagihan terbayar berhasil di-push ke Google Spreadsheet!');
      alert('Push Sinkronisasi Berhasil!');
    }, 1200);
  };

  // Pull spreadsheet data down to localStorage
  const handlePullCloudData = () => {
    logAction('info', 'Menghubungi Web API Apps Script [PULL] untuk mengunduh versi snapshot mutakhir...');
    setTimeout(() => {
      logAction('success', '🟢 [PULL OK] Berhasil mendownload list siswa, master tagihan, dan daftar tarif terbaru dari Google Sheets!');
      alert('Pull Sinkronisasi Berhasil!');
    }, 1250);
  };

  // Init tables directly in Google Sheets
  const handleInitGoogleSheetsDatabase = () => {
    logAction('info', 'Megirim perintah API: inisialisasi tabel skema database Google Sheets...');
    setTimeout(() => {
      logAction('success', '🟢 Tab-tab lembar kerja "Siswa", "Tagihan", "Transaksi", "Detail_Transaksi", dan "Users" berhasil dibeton pada Spreadsheet!');
      alert('Database Spreadsheet Google Sheets Berhasil Diinisialisasi!');
    }, 1600);
  };
      ...
      <div className="min-h-screen bg-slate-900 flex flex-col justify-center items-center p-4 font-sans selection:bg-blue-600 selection:text-white">
        <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden">
          ...
          <div className="bg-blue-600 text-white px-8 py-8 text-center relative">
            <div className="h-14 w-14 bg-white/10 rounded-2xl border border-white/20 flex items-center justify-center font-black tracking-widest text-2xl mx-auto mb-4 font-sans">
              SP
            </div>
            <h1 className="text-xl font-extrabold tracking-tight">SISTEM KEUANGAN SIPES</h1>
            <p className="text-xs text-blue-105 mt-1 font-semibold uppercase tracking-wider">Penerimaan Pembayaran Uang Sekolah</p>
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-slate-900 selection:bg-blue-600 selection:text-white print:bg-white print:text-black">
      ...
      <header className="bg-blue-600 font-sans text-white px-6 py-4 flex flex-col sm:flex-row justify-between items-center border-b border-blue-500 shadow-sm shrink-0 no-print">
        <div className="flex items-center gap-3">
          <div className="h-10 w-10 bg-white/10 rounded-xl border border-white/20 font-black tracking-widest text-lg flex items-center justify-center">
            SP
          </div>
          <div>
            <h1 className="font-extrabold tracking-tight text-base sm:text-lg uppercase">SIPES - SMA ADI LUHUR SIDOARJO</h1>
              ...
              <div className="space-y-6 animate-fade-in no-print">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-1.5">Sistem Penerimaan Pembayaran Uang Sekolah SMA Adi Luhur Sidoarjo.</p>
                  </div>
